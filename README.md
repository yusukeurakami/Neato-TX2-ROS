...
$ ./getKernelSources.sh
```
Some people might be stuck on last line above be shown that the current version of Kernel on TX is not matching with the one you are trying to download from the Nvidia developer's website. I tried to checkout to "vL4T28.2.1" but caused the error saying that the version is not matched.
...
Okay, Now check if ttyAMC module is recognized by TX2 correctly. And get follow.
